...
def build_index(parsed: ParsedDocument, model: str = None, opt=None, doc_path: str = None) -> dict:
    ...
    from ..config import IndexConfig, max_concurrency_scope, llm_params_scope, rpm_limit_scope, get_rpm_limit

    if opt is None:
        opt = IndexConfig(model=model) if model else IndexConfig()

    # Scope the per-index concurrency cap, RPM limit, AND llm kwargs to THIS
    # call only (per thread/async context), so concurrent indexing of other
    # documents isn't affected and a one-off value never sticks as the process
    # default.
    rpm = getattr(opt, "rpm_limit", None) or get_rpm_limit()
